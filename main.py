...
from pyglet import image # biblioteca p/ desenvolvimento de aplicativos/jogos
from pyglet.gl import * # é necessário instalar no CMD
from pyglet.graphics import TextureGroup
from pyglet.window import key, mouse

TICKS_PER_SEC = 60

# Tamanho dos setores usados para facilitar o carregamento do bloco.
SECTOR_SIZE = 16
...
class Window(pyglet.window.Window):
    ...
    def _upd(self, dt):
        ...
        d = dt * speed # distância percorrida.
        dx, dy, dz = self.get_motion_vector()
        # Nova posição no espaço, antes de levar em conta a gravidade.
    ...
    def collide(self, position, height):
        ...
        for face in FACES:  # check all surrounding blocks
            for i in range(3):  # check each dimension independently
                ...
                if d < pad:
                    continue
                for dy in range(height):  # verifique cada altura
                    op = list(np)
                    op[1] -= dy
                    op[i] += face[i]
                    ...
                    if face == (0, -1, 0) or face == (0, 1, 0):
                        # Você está colidindo com o chão ou teto, então pare.
                        # caindo / subindo.
                        self.dy = 0
                    break
        return tuple(p)
    ...
    def set_3d(self):
        """ Configure o OpenGL para desenhar em 3d.
        """
        width, height = self.get_size()
...
def game():
    window = Window(width=800, height=600, caption='Minecraft 2', resizable=True)
    # Oculte o cursor do mouse e evite que o mouse saia da janela.
